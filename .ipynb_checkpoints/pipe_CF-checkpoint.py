...
with col4:
    ...
    width_list = [float(w) for w in selected_w if isinstance(w, (int, float))]
...
# === Проверка и расчёт ===
if pipe_d is None or not width_list:
    st.stop()
...
for w in width_list:
    if w * (1 - overlap / 100) == 0:
        ...
        st.metric(label=f"Tape {w:.0f}\"", value=f"{format(tape_length, ',.1f')} mm", delta=f"Tape {w:.0f}\"", label_visibility="collapsed")
